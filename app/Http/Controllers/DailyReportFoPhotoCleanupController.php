<?php
// app/Http/Controllers/DailyReportFoPhotoCleanupController.php

namespace App\Http\Controllers;

use App\Models\DailyReportFoPhotoCleanupLog;
use App\Services\DailyReportFoPhotoCleanupService;
use Illuminate\Support\Carbon;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class DailyReportFoPhotoCleanupController extends Controller
{
    public function __construct(
        private DailyReportFoPhotoCleanupService $service
    ) {}

    /**
     * Halaman utama:
     * - Stats storage saat ini
     * - Preview foto yang akan dihapus
     * - Riwayat penghapusan
     */

    /**
     * UPDATE index() — tambah filter history
     * Ganti method index() yang sudah ada dengan ini
     */
    ...
    public function index(Request $request)
    {
        ...
        $logsQuery = DailyReportFoPhotoCleanupLog::with('executor')
        ...
        return view('superadmin.daily-report-fo.photo-cleanup.index', [
            'preview'         => $preview,
    ...
    public function execute(Request $request)
    {
        ...
        return redirect()
            ->route('superadmin.daily-report-fo.photo-cleanup.index')
    ...
    public function previewRange(Request $request)
    {
        ...
        $preview      = $this->service->preview();
        $storageStats = $this->service->storageStats();
        $logs         = DailyReportFoPhotoCleanupLog::with('executor')
            ->orderBy('executed_at', 'desc')
            ->paginate(10);
